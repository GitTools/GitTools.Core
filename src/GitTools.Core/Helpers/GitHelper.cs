// --------------------------------------------------------------------------------------------------------------------
// <copyright file="GitHelper.cs" company="GitTools">
...
namespace GitTools
{
    ...
    public static class GitHelper
    {
        ...
        private static void CreateFakeBranchPointingAtThePullRequestTip(Repository repo)
        {
            ...
            if (refs.Count == 0)
            {
                Log.ErrorAndThrowException<GitToolsException>("Couldn't find any remote tips from remote '{0}' pointing at the commit '{1}'.", remote.Url, headTipSha);
            }

            if (refs.Count > 1)
            {
                var names = string.Join(", ", refs.Select(r => r.CanonicalName));
                Log.ErrorAndThrowException<GitToolsException>("Found more than one remote tip from remote '{0}' pointing at the commit '{1}'. Unable to determine which one to use ({2}).", remote.Url, headTipSha, names);
            }
            ...
            {
                Log.ErrorAndThrowException<GitToolsException>("Remote tip '{0}' from remote '{1}' doesn't look like a valid pull request.", canonicalName, remote.Url);
            }
        ...
        private static void CreateMissingLocalBranchesFromRemoteTrackingOnes(IRepository repo, string remoteName)
        {
        ...
        private static Remote EnsureOnlyOneRemoteIsDefined(IRepository repo)
        {
            ...
            Log.ErrorAndThrowException<GitToolsException>("{0} remote(s) have been detected. When being run on a TeamCity agent, the Git repository is expected to bear one (and no more than one) remote.", howMany);
